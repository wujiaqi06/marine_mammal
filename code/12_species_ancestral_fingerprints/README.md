# Fig. 6 and Supplementary Fig. S3 workflows

This directory contains the current analysis additions to the frozen endpoint-
fix release. No scientific values were changed when the scripts were copied
into the public reproducibility package.
...
`MARINE_MAMMAL_OUTPUT_ROOT` before running:

```bash
Rscript code/12_species_ancestral_fingerprints/run_terminal_only_projection.R --cores=12
...
`run_terminal_only_projection.R` generates descriptive internal-branch
