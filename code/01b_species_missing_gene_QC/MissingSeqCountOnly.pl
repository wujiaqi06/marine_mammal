...
#print "@species_name\n";
open MISS, ">.././$output_folder.MissingInfo.txt" or die $!;
open MISSCOUNT, ">.././$output_folder.MissingCount.txt" or die $!;
open MISSSPECIESCOUNT, ">.././$output_folder.SpeciesMissingCount.txt" or die $!;
print MISSCOUNT "Sequence\tMissingNumber\n";
print MISS "Genes\tMissingSpecies\n";
print MISSSPECIESCOUNT "Species\tMissingNumber\n";
my %species_missing;
foreach my $seq (@sequences){
	...
	foreach my $species (@species_name){
		if (!(exists ($read_sequence{$species}))){
			print MISS "$species\t";
			$count ++;
			if (exists $species_missing{$species}){
				$species_missing{$species} ++;
			} else {
				$species_missing{$species} = 1;
			}
		} else {
...
foreach my $i (sort keys %species_missing){
	print MISSSPECIESCOUNT "$i\t$species_missing{$i}\n";
}
