...
manifest_file <- "07_ttest_screening/preflight/manifests/endpointfix_ttest_run_manifest.proposed.tsv"
...
qc_dir <- "07_ttest_screening/qc"
minimal_dir <- "07_ttest_screening/outputs_minimal"
dir.create(qc_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(minimal_dir, recursive = TRUE, showWarnings = FALSE)

script_file <- "07_ttest_screening/scripts/run_ttest_screening.R"
...
write_tsv(input_summary, file.path(qc_dir, "endpointfix_stage1_ttest_input_summary.tsv"))
...
write_tsv(shape_summary, file.path(qc_dir, "endpointfix_stage1_ttest_shape_summary.tsv"))
...
make_trait_anc <- function(trait_column) {
  ...
  node_states <- max.col(anc_results$ancestral_likelihoods)
...
for (idx in seq_along(run_ids)) {
  ...
  out_dir <- file.path("07_ttest_screening/outputs", run_id)
...
write_tsv(do.call(rbind, trait_state_rows), file.path(qc_dir, "endpointfix_stage1_trait_state_branch_summary.tsv"))
write_tsv(do.call(rbind, testability_rows), file.path(qc_dir, "endpointfix_stage1_gene_testability_summary.tsv"))
write_tsv(do.call(rbind, skipped_rows), file.path(qc_dir, "endpointfix_stage1_skipped_gene_list.tsv"))
write_tsv(do.call(rbind, screening_rows), file.path(qc_dir, "endpointfix_stage1_screening_summary.tsv"))
write_tsv(do.call(rbind, direction_rows), file.path(qc_dir, "endpointfix_stage1_direction_summary.tsv"))
write_tsv(numeric_leak, file.path(qc_dir, "endpointfix_stage1_ttest_numeric_leak_check.tsv"))

commands <- paste0(
  "Rscript 07_ttest_screening/scripts/run_ttest_screening.R 07_ttest_screening/preflight/manifests/endpointfix_ttest_run_manifest.proposed.tsv ",
  run_ids,
  " 07_ttest_screening/outputs/",
  run_ids
)

summary <- c(
  "# Endpoint-fix Stage 1 t-test Run Summary",
  "",
  paste("Generated:", format(Sys.time(), "%Y-%m-%d %H:%M:%S %z")),
  "",
  "## Scope",
  "Stage 1 fix baseline/comparison t-test only. No free t-test, drop/sensitivity t-test, LASSO, enrichment, figures, or old-vs-new biological comparison were run.",
  "",
  ...
  "",
  "## Logic",
  "Old t-test logic, ancestral reconstruction, branch exclusion of 0.5, BH/FDR while-loop rule, terminal mean imputation, and output naming were reused without design change. The only script patch was IO-only: GBI matrix reading now uses `header = TRUE` so old-label branch headers are not read as a gene row.",
  "",
  "## IO Smoke Check",
  ...
  "",
  "## Codex self-check",
  "Codex self-check: Stage 1 fix baseline/comparison t-test completed with no obvious STOP-level QC failure. Final t-test PASS requires review by marine mammal rerun control chat."
)
writeLines(summary, file.path(qc_dir, "endpointfix_stage1_ttest_run_summary.md"))
